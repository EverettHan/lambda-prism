...
As lambda functions are running on Amazon Linux 2 VMs, we need create the same VM on local Windows machine so that we can do some local testing.
 ...
 ## Unzip the zip file to a temp folder, and rename the exe file name from AL2022.exe to a name you want for your distribution under wsl, such as Amazon2.exe
