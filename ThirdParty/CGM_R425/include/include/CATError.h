...
class ExportedByJS0ERROR CATError : public CATBaseUnknown {
  ...
  public:
     ...
     * However, when compiled in setjmp/longjmp mode (NATIVE_EXCEPTION not defined), 
     * such optimizations can break invalid code relying on undefined behavior. For e.g., code 
     * modifying a NON-volatile local variable in CATTry and attempting to read it CATCatch blocks.
     */
#if defined(NATIVE_EXCEPTION)
    [[noreturn]]
#endif
    void Throw (const char *loc, int line, void * = nullptr);
    /** @nodoc Used by macros in CATErrorMacros.h in NATIVE_EXCEPTION mode */
    [[noreturn]] void ThrowCxx (const char *loc, int line);
    ...
    /** @nodoc Handle lifecycle of CATError during ThrowCxx - System internal use only!!! */
